...
public class ComSQLiteDatabasePool {
    ...
    private ComSQLiteDatabase newSQLiteDatabase() {

        // 创建一个数据库连接
        ComSQLiteDatabase comSQLiteDatabase = new ComSQLiteDatabase(context, params);
    ...
    public synchronized ComSQLiteDatabase getSQLiteDatabase() {
        // 确保连接池己被创建
        if (pSQLiteDatabases == null) {
            return null; // 连接池还没创建，则返回 null
        }

        ComSQLiteDatabase sqliteDatabase = getFreeSQLiteDatabase(); // 获得一个可用的数据库连接
    ...
    private ComSQLiteDatabase getFreeSQLiteDatabase() {
        // 从连接池中获得一个可用的数据库连接
        ComSQLiteDatabase sqLiteDatabase = findFreeSQLiteDatabase();
    ...
    private ComSQLiteDatabase findFreeSQLiteDatabase() {
        ComSQLiteDatabase sqliteDatabase = null;
     ...
     */
    private boolean testSQLiteDatabase(ComSQLiteDatabase sqliteDatabase) {
    ...
    public void releaseSQLiteDatabase(ComSQLiteDatabase sqLiteDatabase) {
    ...
    private void closeSQLiteDatabase(ComSQLiteDatabase sqlLiteDatabase) {
    ...
    class PooledSQLiteDatabase {
        ComSQLiteDatabase sqliteDatabase = null;// 数据库连接
        boolean busy = false; // 此连接是否正在使用的标志，默认没有正在使用

        // 构造函数，根据一个 TASQLiteDatabase 构告一个 PooledSQLiteDatabase 对象
        public PooledSQLiteDatabase(ComSQLiteDatabase sqliteDatabase) {
            this.sqliteDatabase = sqliteDatabase;
        }

        // 返回此对象中的连接
        public ComSQLiteDatabase getSqliteDatabase() {
            return sqliteDatabase;
        }

        // 设置此对象的，连接
        public void setSqliteDatabase(ComSQLiteDatabase sqliteDatabase) {
            this.sqliteDatabase = sqliteDatabase;
        }
