...
import com.android.common.db.*;
import com.android.common.db.ComSQLiteDatabase.DBParams;
 ...
 */
public class ComSQLiteDatabasePool {
    ...
    private static HashMap<String, ComSQLiteDatabasePool> poolMap = new HashMap<String, ComSQLiteDatabasePool>();
     ...
     */
    public synchronized static ComSQLiteDatabasePool getInstance(Context context, DBParams params, Boolean isWrite) {
        String dbName = params.getDbName().trim();
        ComSQLiteDatabasePool pool = poolMap.get(dbName);
        if (pool == null) {
            pool = new ComSQLiteDatabasePool(context, params, isWrite);
     ...
     * @param context 上下文
     */
    public static ComSQLiteDatabasePool getInstance(Context context) {
     ...
     * @param isWrite   打开数据库如果是 isWrite为true,则磁盘满时抛出错误
     */
    public static ComSQLiteDatabasePool getInstance(Context context, String dbName, int dbVersion, Boolean isWrite) {
     ...
     * @param isWrite 打开数据库如果是 isWrite为true,则磁盘满时抛出错误
     */
    public ComSQLiteDatabasePool(Context context, DBParams params, Boolean isWrite) {
    ...
    private void createSQLiteDatabase(int numSQLiteDatabase) {
        ...
        for (int x = 0; x < numSQLiteDatabase; x++) {
            ...
            try {
                ...
                Logger.i(" 创建数据库连接失败！ " + e.getMessage());
            }
            Logger.i("数据库连接己创建 ......");
        }
    }
    ...
    private com.android.common.db.ComSQLiteDatabase newSQLiteDatabase() {

        // 创建一个数据库连接
        com.android.common.db.ComSQLiteDatabase comSQLiteDatabase = new com.android.common.db.ComSQLiteDatabase(context, params);
        comSQLiteDatabase.openDatabase(isWrite);
        return comSQLiteDatabase; // 返回创建的新的数据库连接
    }
    ...
    public synchronized com.android.common.db.ComSQLiteDatabase getSQLiteDatabase() {
        // 确保连接池己被创建
        if (pSQLiteDatabases == null) {
            return null; // 连接池还没创建，则返回 null
        }

        com.android.common.db.ComSQLiteDatabase sqliteDatabase = getFreeSQLiteDatabase(); // 获得一个可用的数据库连接

        // 如果目前没有可以使用的连接，即所有的连接都在使用中
        while (sqliteDatabase == null) {
            Logger.i("没有可用的数据库连接，等一会儿再试");
            // 等一会再试
            wait(250);
    ...
    private com.android.common.db.ComSQLiteDatabase getFreeSQLiteDatabase() {
        // 从连接池中获得一个可用的数据库连接
        com.android.common.db.ComSQLiteDatabase sqLiteDatabase = findFreeSQLiteDatabase();
    ...
    private com.android.common.db.ComSQLiteDatabase findFreeSQLiteDatabase() {
        com.android.common.db.ComSQLiteDatabase sqliteDatabase = null;
     ...
     */
    private boolean testSQLiteDatabase(com.android.common.db.ComSQLiteDatabase sqliteDatabase) {
    ...
    public void releaseSQLiteDatabase(com.android.common.db.ComSQLiteDatabase sqLiteDatabase) {
        // 确保连接池存在，如果连接没有创建（不存在），直接返回
        if (pSQLiteDatabases == null) {
            Logger.d(" 连接池不存在，无法返回此连接到连接池中 !");
            return;
        }
        PooledSQLiteDatabase pSqLiteDatabase = null;
    ...
    private void closeSQLiteDatabase(com.android.common.db.ComSQLiteDatabase sqlLiteDatabase) {
    ...
    class PooledSQLiteDatabase {
        com.android.common.db.ComSQLiteDatabase sqliteDatabase = null;// 数据库连接
        boolean busy = false; // 此连接是否正在使用的标志，默认没有正在使用

        // 构造函数，根据一个 TASQLiteDatabase 构告一个 PooledSQLiteDatabase 对象
        public PooledSQLiteDatabase(com.android.common.db.ComSQLiteDatabase sqliteDatabase) {
            this.sqliteDatabase = sqliteDatabase;
        }

        // 返回此对象中的连接
        public com.android.common.db.ComSQLiteDatabase getSqliteDatabase() {
            return sqliteDatabase;
        }

        // 设置此对象的，连接
        public void setSqliteDatabase(com.android.common.db.ComSQLiteDatabase sqliteDatabase) {
            this.sqliteDatabase = sqliteDatabase;
        }
